...
const Navbar = async () => {
     ...
     return (
          <div className="fixed top-0 h-fit bg-slate-800 border-b border-slate-700 z-[50] py-2 w-full">
              <div className="h-12 w-full flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 right-0 p-4">
                      {session?.user ? (
                          <UserAccountNav user={session.user} username={session.user.username} role={await getRoleAsync(session.user)}  style={{ marginRight: '50px'}} />
                      ) : (
